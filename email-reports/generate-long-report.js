...
async function fetchForDate(date, usdToInr) {
  ...
  console.log(`   ✅ Data fetched for ${date}`);
  ...
  console.log(`   📥 Downloads: Android ${androidDownloads ?? 'N/A'}, iOS ${iosDownloads ?? 'N/A'}`);
  console.log(`   📺 Views: Android ${androidViews ?? 'N/A'}, iOS ${iosViews ?? 'N/A'}`);
  ...
  if (androidRevUsd || iosRevUsd) {
    console.log(`   💰 AdMob: Android $${(androidRevUsd || 0).toFixed(2)} × ${usdToInr} = ₹${androidRevInr ?? 0}, iOS $${(iosRevUsd || 0).toFixed(2)} × ${usdToInr} = ₹${iosRevInr ?? 0}`);
  }
  ...
  if (rzRevenue) {
    console.log(`   💳 Razorpay: ₹${rzRevenue} (${dayPayments.length} payments)`);
  }
...
async function generateExcel(rows) {
  ...
  const ws = wb.addWorksheet(
    `Last ${process.env.EXCEL_REPORT_DAYS_COUNT} days`
  );
  ...
  rows.forEach((r) => {
    const row = ws.addRow({
      ...
      admobRevAndroid:
        r.admobRevAndroid !== null ? `₹${r.admobRevAndroid}` : "N/A",
  ...
  const sum = (arr, pick) =>
    arr.reduce((s, x) => s + (Number.isFinite(pick(x)) ? pick(x) : 0), 0);
  ...
  const out = path.join(
    __dirname,
    `${process.env.EXCEL_REPORT_DAYS_COUNT}-days-report.xlsx`
  );
  ...
  console.log(`   Total Downloads: ${tDownloads.toLocaleString()}`);
  console.log(`   Total Episode Views: ${tViews.toLocaleString()}`);
  console.log(`   Total AdMob Impressions: ${tImpr.toLocaleString()}`);
  console.log(`   Total AdMob Revenue: ₹${tAdm.toLocaleString()}`);
  console.log(`   Total Razorpay Revenue: ₹${tRz.toLocaleString()}`);
...
async function main() {
  ...
  console.log(`${"=".repeat(60)}`);
  console.log(
    `\n📊 Generating last ${process.env.EXCEL_REPORT_DAYS_COUNT} days report (IST)...`
  );
  ...
  console.log(`\n📅 Date Range:`);
  console.log(`   From: ${dates[dates.length - 1]}`);
  console.log(`   To: ${dates[0]}`);
  console.log(`   Total Days: ${dates.length}`);

  console.log(`\n${"=".repeat(60)}`);
  console.log(`📡 FETCHING DATA FOR ${dates.length} DAYS`);
  console.log(`${"=".repeat(60)}`);
  ...
  for (const d of dates) {
    const row = await fetchForDate(d, usdToInr);
    result.push(row);
    fetchedCount++;
    console.log(`   Progress: ${fetchedCount}/${dates.length} days completed`);
